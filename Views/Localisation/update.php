...
if (!isset($_GET["token"]) && !is_numeric($_GET["token"])) {
    ...
    foreach ($locations->get_one("id_localisation", base64_decode($_GET["token"])) as $data) {
...
<body>
    ...
    <div id="container">
        <div id="space"></div>
        <form action="<?= parse('../../Controllers/Localisation/Controller.php') ?>" method="POST" autocomplete="off"
            id="auth-form">
            <div class="div">
                <h2>Modifier <strong><u><?= $adresse ?></u></strong></h2>
            </div>
            <div class="div">
                <input type="text" name="intitule" id="intitule" value="<?= $adresse ?>" class="div-input"
