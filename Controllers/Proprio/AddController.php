                                        ...
                                        if (isset($_COOKIE['role']) && base64_decode($_COOKIE['role']) === 'proprietaire') {
                                            ...
                                            if ($_POST['nom']) {
                                                if (checkData('name', $_POST['nom'])) {
                                                    ...
                                                    if ($biens->create($id_localisation, $iduser, $id_type, $name, $info, $prix)) {
                                                ...
                                                if ($biens->create($id_localisation, $iduser, $id_type, null, $info, $prix)) {
                                                    $_SESSION['returnValue'] = 'ok';
                                                } else {
                                                    $alert = new Alert('Une erreur est survenue ):', $_SERVER['HTTP_REFERER']);
                                                    $alert->show_message();
                                            ...
                                            if (isset($_SESSION['returnValue']) && $_SESSION['returnValue'] === 'ok') {
                                                ...
                                                foreach ($_FILES['files']['name'] as $key => $value) {
                                                    ...
                                                    $new_name = $_COOKIE['login'] . '_' . time() + 1 . '_' . $_SESSION['id_bien'];
                                                    $new_rep = '../../houses/';
                                                    ...
                                                    if ($images->create($_SESSION['id_bien'], $file_name, $file_size, $file_type) && move_uploaded_file($tmp, $new_tmp)) {
                                                ...
                                                if (isset($_SESSION['finally']) && $_SESSION['finally'] === true) {
                                                    $alert = new Alert('La tache a été effectuée avec succès :)', '../../Views/proprio/index.php');
                                                    $alert->show_message();
                                                } else {
                                                    $alert = new Alert("Une erreur s\'est produite :(", $_SERVER['HTTP_REFERER']);
                                                    $alert->show_message();
                                                ...
                                                $alert = new Alert("Une erreur s\'est produite ICI yo ):", $_SERVER['HTTP_REFERER']);
                                                $alert->show_message();
                                            }
